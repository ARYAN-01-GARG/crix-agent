...
export async function taskRoutes(app: FastifyInstance, opts: TaskRouteOptions): Promise<void> {
  ...
  app.post<{ Body: TaskBody }>("/api/tasks", async (request, reply) => {
    ...
    await queue.add(taskId, payload, { jobId: taskId, removeOnComplete: 100, removeOnFail: 50 });
